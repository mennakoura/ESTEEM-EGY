...
2. **Rename the workbook to `Modelsheets.xlsx`.** The script hard-codes the filename:
   ...
   The supplied file is named `ESTEEM-EGY_ModelSheets.xlsx`, so either rename it to `Modelsheets.xlsx` or edit line 14 of the script. (Filename matching is case-sensitive on Linux/macOS.)
...
- [ ] Workbook renamed to `Modelsheets.xlsx` (or line 14 edited) and in the working directory.
